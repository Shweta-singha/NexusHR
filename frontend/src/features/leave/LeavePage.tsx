...
export default function LeavePage() {
  ...
  return (
    <div>
      <h1 className="mb-4 text-2xl font-semibold text-slate-900">Leave</h1>
      ...
      {balanceQuery.data && (
        <div className="mb-6 grid grid-cols-4 gap-4">
          {[
            ...
            <div key={b.label} className="rounded-lg border border-slate-200 bg-white p-4">
              <p className="text-xs text-slate-500">{b.label}</p>
              <p className="text-xl font-semibold text-slate-900">{b.value}</p>
            </div>
          ))}
        </div>
      )}
